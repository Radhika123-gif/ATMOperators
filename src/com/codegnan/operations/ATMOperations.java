...
import com.codegnan.customExceptions.IncorrectPinLimitReachedExceptions;
...
import com.codegnan.customExceptions.InvalidCardExceptions;
...
public class ATMOperations {
	...
	public static IATMService validateCard(long cardNumber) throws InvalidCardExceptions {
		if (dataBase.containsKey(cardNumber)) {
			return dataBase.get(cardNumber);
		} else {
			ACTIVITY.add("Accessed by : " + cardNumber + " is not compatiable");
			throw new InvalidCardExceptions("this is not a valid card");
		}
	...
	public static void resetUserAttempts(IATMService operatorCard) {
		...
		} catch (InvalidCardExceptions ive) {
	...
	public static IATMService validCredentials(long cardNumber, int pinNumber)
			throws InvalidCardExceptions, InvalidPinException, IncorrectPinLimitReachedExceptions {
		if (dataBase.containsKey(cardNumber)) {
		...
		if (card.getChances() >= 3) {
			throw new IncorrectPinLimitReachedExceptions("you have reached wrong limit of pin number which is 3  attempts ");
		}
		if (card.getPinNumber() != pinNumber) {
						...
						while(true) {
							...
							try {
								...
								System.out.println("Do you want to continue?(y/n)");
									...
									}catch(InvalidPinException|InvalidCardExceptions|IncorrectPinLimitReachedExceptions e) {
